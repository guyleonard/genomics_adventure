...
Digital normalisation reduces coverage by counting k-mers across your reads and discarding those in excess of a chosen depth.  By trimming highly abundant sequences, it keeps the size of the dataset manageable without losing much diversity.  This is especially helpful when a sequencing run produced far more data than is needed for assembly.

One tool that implements this is [BBNorm](https://jgi.doe.gov/data-and-tools/software-tools/bbtools/bb-tools-user-guide/bbnorm-guide/) :mag: from the BBTools package.  It filters reads to a specified coverage using a k-mer count table.  A typical run on paired Illumina reads might look like:

```bash
bbnorm.sh in=read_1.fq.gz in2=read_2.fq.gz \
    out=norm_1.fq.gz out2=norm_2.fq.gz \
    target=40 min=5 threads=8
```

`target` sets the desired coverage after normalisation and `min` retains reads below this depth.  Digital normalisation speeds up assembly and reduces memory usage, but overly strict thresholds can discard rare sequences.  Use it when coverage is very high and computational resources are limiting, but avoid it if detecting low-abundance variants is critical.
